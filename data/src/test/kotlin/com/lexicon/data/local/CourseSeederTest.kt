...
class CourseSeederTest {
    ...
    fun `an empty database is filled from the asset`() =
        runTest {
            ...
            coVerify { courseDao.replaceCatalog(any(), match { it.size == 2 }, any(), any(), any()) }
        }
    ...
    fun `a changed asset replaces the catalogue`() =
        runTest {
            ...
            coEvery { courseDao.replaceCatalog(any(), capture(lessons), any(), any(), any()) } returns Unit
    ...
    fun `an unchanged asset is not re-imported`() =
        runTest {
            ...
            coVerify(exactly = 0) { courseDao.replaceCatalog(any(), any(), any(), any(), any()) }
        }
    ...
    fun `a lesson keeps its words in the order the book introduces them`() =
        runTest {
            ...
            coEvery { courseDao.replaceCatalog(any(), any(), any(), capture(words), any()) } returns Unit
